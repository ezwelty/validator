...
schema = Schema(
    {
        ...
        Table('main'): {
            ...
            Column('string_enum'): [Check.in_list(['a', 'b'])],
            Column('string_regex'): [Check.matches_regex(r'[ab]')],
        },
        Table('secondary'): {
            Table(): [
                Check.has_columns(
                    ['integer_pk', 'string_pk', 'boolean_lookup', 'extra'], fill=True
                ),
            ...
            Column('string_pk'): [Check.not_null()],
            Column('boolean_lookup'): [Check.parse_as_type('boolean')],
            Table(): [
                Check.unique_rows(['integer_pk', 'string_pk']),
                Check.matches_foreign_columns(
                    table='main',
                    join={'integer_pk': 'integer_pk'},
                    columns={'boolean_lookup': 'boolean'},
                ),
            ],
        },
    }
...
failures = pd.DataFrame(
    [
        ...
        },
        {
            'table': 'secondary',
            'column': ['extra'],
            'check': Check.only_has_columns(
                ['integer_pk', 'string_pk', 'boolean_lookup']
            ),
        },
        {
            'table': 'secondary',
        ...
        {
            'table': 'secondary',
            'check': Check.matches_foreign_columns(
                table='main',
                join={'integer_pk': 'integer_pk'},
                columns={'boolean_lookup': 'boolean'},
            ),
...
def test_checks_child_column_without_required_parents() -> None:
    ...
    reports = [
        ...
        schema(
            dfs[table][column], name=Column(column, table), target=Column(column, table)
        ),
    ]
...
def test_checks_tables() -> None:
    ...
    reports = [schema(dfs, name=Tables()), schema(dfs, name=Tables(), target=Tables())]
    assert all(r.target.equals(report.target) for r in reports)
    assert all(r.counts == report.counts for r in reports)
