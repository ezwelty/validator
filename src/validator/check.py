...
from .helpers import Timer, filter_kwargs, stringify_call
...
def _test_inputs(
    fn: Callable, inputs: Dict[str, Type[Target]] = None
) -> Dict[str, Type[Target]]:
    ...
    invalid = [value for value in mapped.values() if not issubclass(value, Target)]
    if invalid:
...
def _test_required(
    ...
    if not_children:
        raise ValueError(f'Required {not_children} not children of {parents}')
    return required
...
class Check:
    ...
    def _test_call(
        ...
        if target is None:
            ...
            raise ValueError(f'Target class does not match check target ({cls})')
    ...
    def serialize(self) -> Dict[str, Any]:
        ...
        for attr in ['message', 'tag']:
            ...
            if value is not None and (attr not in defaults or value != defaults[attr]):
                result[attr] = value
        return result
...
class Result:
    ...
    def code(self) -> Literal['pass', 'fail', 'error', 'skip']:
        ...
        if self.error is not None:
            return 'error'
        if self.missing is not None or (self.valid is None and self.output is None):
            return 'skip'
        if (
            self.valid is True
    ...
    def row(self) -> Optional[List[Hashable]]:
        ...
        return (self.axis == 'row' and list(self.valid[~self.valid].index)) or None
    ...
    def value(self) -> Optional[List[Hashable]]:
        ...
        return (
            self.axis == 'row'
            and isinstance(self.target, Column)
            and list(self.input[self.valid[~self.valid].index])
        ) or None
    ...
    *,
    ...
    def factory(fn: Callable) -> Callable:
        ...
        if hasattr(Check, method_name):
            raise ValueError(f'{Check.__name__}.{method_name} already exists')
        ...
        @_generate_method(fn, inputs=inputs, required=required, test=test, axis=axis)
