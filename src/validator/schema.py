...
class Schema:
    ...
    def _flatten(checks: SchemaDict) -> FlatSchemaDict:
        ...
        def traverse(
            checks: SchemaDict, baseprefix: str = '', basenames: dict = {}
        ) -> None:
                ...
                if isinstance(value, list):
                    for i, check in enumerate(value):
                        if isinstance(check, Check):
                            ...
                            errors.append(
                                f'{prefix}[{i}]: Expected Check, not {type(check)}'
                            )
    ...
    def __call__(
        ...
        inputs = extract_data(
            copylib.deepcopy(data) if copy else data, name=name, target=target
        )
        if copy:
                ...
                if isinstance(target, (Tables, Table)) and isinstance(key, Column):
                    if key.column not in args[Table]:
                        ...
                        results[key] = Result(
                            check,
                            target=key,
                            missing=[Column(key.column, table=key.table)],
                        )
                        continue
                # Run check
        ...
        return Report(list(results.values()), target=target, input=input, output=output)
...
class Report:
    ...
    def empty(self) -> bool:
        ...
        """
        return not self.results or all(result.code == 'skip' for result in self.results)
    ...
    def to_dataframe(self, explode: bool = False) -> pd.DataFrame:
        ...
        if explode:
            ...
            df = pd.concat(
                [df[~mask].explode(['row', 'value']), df[mask].explode('row')]
            )
    ...
    def project(self, data: Data, name: Target = None) -> Tuple['Report', 'Report']:
        ...
        for result in self.results:
            ...
            if isinstance(result.target, Tables):
                ...
                isinstance(result.target, (Table, Column))
                and result.check.axis == 'row'
            ):
                index = input.index
            else:
                        ...
                        **base,
                        ...
                        if result.input is None
                        else (
                            {
                                ...
                                for key, isin in zip(result.input, input_mask)
                                if isin
                            }
                            if isinstance(result.input, dict)
                            else result.input[input_mask]
                        ),
                    )
                )
                outs.append(
