...
def has_sorted_columns(
    df: pd.DataFrame, *, columns: Sequence[Hashable], sort: bool = False
) -> Dict[Hashable, bool]:
    """
...
@register_check(
    ...
    required=lambda columns: [
        Column(column) for column in [*columns, *columns.values()]
    ],
)
...
@register_check(
    ...
    required=lambda table, join, columns: (
        ...
        + [
            ...
            for column in [*join.values(), *columns.values()]
        ]
    ),
)
def matches_foreign_columns(
    ...
    *,
    ...
    valid = joined.groupby('__row__', sort=False).apply(
        lambda g: (
...
def rename_columns(
    ...
    """
    valid = {
        name: name not in df or rename not in df for name, rename in columns.items()
    }
...
def columns_missing_or_null(
    ...
    """
    return {
        column: column not in df or not df[column].notnull().any() for column in columns
    }
